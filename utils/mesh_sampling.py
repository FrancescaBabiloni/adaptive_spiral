...
if __name__ == "__main__":
    ...
    ds_factors = [4, 4, 4, 4]
    template_fp = './flame_template'
    # generate/load transform matrices
    transform_fp = osp.join(template_fp, 'head_template_transform_{}.pkl'.format(ds_factors[-1]))
    if not osp.exists(transform_fp):
        print('Generating transform matrices...')
        import pdb
        pdb.set_trace()
        mesh = Mesh(filename=osp.join(template_fp,'head_template.obj'))
